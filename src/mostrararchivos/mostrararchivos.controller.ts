...
export class MostrararchivosController {
  ...
  @Get('imagen_facultad/:url_imagen')
  getImagen2(@Param('url_imagen') filename: string, @Res() res: Response) {

    // Construye la ruta completa del archivo usando el directorio actual de trabajo
    const filePath = path.join(__dirname, '..', '..', 'uploads', 'facultadesimagenes', filename);
